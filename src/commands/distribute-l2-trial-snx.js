...
async function distributeSNX({
	...
	for (let i = 0; i < numAccounts; i++) {
		...
		if (distributed === '0' && escrowed !== '0' && balance === '0') {
			...
			console.log(gray(`    ⮑  Sending ${rewardAmount} SNX`));

			const result = await runTx({
				txPromise: Synthetix.transfer(account, parseEther(`${rewardAmount}`), overrides),
				provider,
			});
			if (!result.success) {
...
program
	...
	.option('--gas-price <value>', 'Gas price to set when performing transfers', '1')
	.action(async (...args) => {
		try {
